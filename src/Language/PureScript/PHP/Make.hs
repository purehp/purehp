...
import           Language.PureScript.PHP.CodeGen.Common (phpModuleName, atomModuleName, atom, ModuleType(..))
...
buildActions outputDir env foreigns usePrefix = do
  ...
  where
    ...
    codegen m = do
      ...
      (exports, rawPHP) <- moduleToPHP env m foreignExports
      ...
      let php :: T.Text = T.unlines $ map ("% " <>) prefix ++ directives ++ [ pretty ]
